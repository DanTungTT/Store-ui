...
const BenefitItem = ({ img, type, name, description, list }) => {
    const bgBtns = {
        Cross: "bg-[#0046ab]",
        ...
        <>
            <div className="w-[30rem] shadow-[0_0_.1rem_#333]">
                ...
                <div>
                    ...
                    <p className={clsx(styles.headingColor, "text-[1.3rem] pl-8 pr-10 pb-10 h-[5rem] mb-13")}>
                        {description}
                    </p>
                    <ul className="list-disc [&>li]:my-4 [&>li::marker]:text-orange-500 pl-15 pr-10 text-[1.2rem] min-h-[15.6rem]">
                        {list.map((item, index) => (
                            <li key={index} className={styles.headingColor}>
                                {item}
                            </li>
                        ))}
                    </ul>
